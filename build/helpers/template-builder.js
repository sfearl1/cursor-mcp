...
import { PROJECT_ROOT, resolveRoot, resolveSrc } from '../paths.js';
export class TemplateBuilder {
    ...
    constructor(rootDir) {
        if (rootDir) {
            ...
            // Use the project root by default
            this.rootDir = PROJECT_ROOT;
            console.error('Using project root:', this.rootDir);
        }
    }
    ...
    async runRepomix(files) {
        try {
            ...
            const outputPath = resolveRoot('output.xml');
            ...
            const result = await pack(pathsToProcess, packOptions);
    ...
    async compile(task, code, rulesPath, agentType = 'architect') {
        try {
            ...
            try {
                ...
                try {
                    ...
                    if (rulesPath) {
                        rulesContent = await fs.readFile(resolveRoot(rulesPath), 'utf-8');
                    }
                    else {
                        const defaultRulesPath = resolveSrc('cursor-template', 'rules.md');
